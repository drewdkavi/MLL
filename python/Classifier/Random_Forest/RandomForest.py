...
from Classifier.Random_Forest import generateRuleRF
...
class RandomForest(Classifier):
    ...
    def _make_tree(self, x_train: npt.NDArray[npt.NDArray[float]], y_train: npt.NDArray[int | float]):
        ...
        # TODO: Make this a true stack? e.g. using linked list (poss. python deque?)
        ...
        # print(f"Tree height: {get_sub_tree_height(root)}")
        return root
    ...
    def predict(self, x_dp: npt.NDArray[float]) -> int | float | str:
        ...
        top_prediction_count: int = 0
        top_prediction_category: int = -1
        preds = np.zeros(self._num_classes)

        for i in range(self.num_estimators):
            estimator_prediction = self._predict_tree(x_dp, i)
            preds[estimator_prediction] += 1
            if preds[estimator_prediction] > top_prediction_count:
                top_prediction_category = estimator_prediction
                top_prediction_count = preds[estimator_prediction]

        return top_prediction_category
