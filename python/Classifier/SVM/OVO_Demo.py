...
from Binary_SVM import BinarySVMLinear, BinarySVMLinearSeparable, SVMLinearOVR, SVMLinearOVRSeparable, SVMLinearOVO
...
svm = SVMLinearOVO(input_dim=NUM_FEATURES, num_classes=4)
...
def separator(x1_dp, i, j):
    # print(f"Creating a separator for w[{i}] = {w[i]}")
    return (-1 / w[i][j][1]) * (x1_dp * w[i][j][0] + w[i][j][2])
...
for i in range(4):
    for j in range(4):
        if 3 - i != j:
            line = [separator(x1, 3 - i, j) for x1 in x1s]
            ax.plot(x1s, line, to_colour[i])
